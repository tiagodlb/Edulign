...
import { useAuth } from "@/components/AuthProvider" // Fix import to use the correct AuthProvider
...
interface NavItem {
  ...
  badge?: string | number
  description?: string
  requiresRole?: "ADMIN" | "PROFESSOR" | "ALUNO"
  requiresPermission?: string
}

const getNavItems = (role: "ADMIN" | "PROFESSOR" | "ALUNO" | null): NavItem[] => {
...
export function MainNav() {
  ...
  const navItems = React.useMemo(() => {
    return getNavItems(user?.tipo || null)
  }, [user?.tipo])

  // Get app title based on role
  const appTitle = React.useMemo(() => {
    if (user?.tipo === "ADMIN") return "Edulign - Admin"
    if (user?.tipo === "PROFESSOR") return "Edulign - Professor"
    return "Edulign"
  }, [user?.tipo])
  ...
  const pendingCount = React.useMemo(() => {
    ...
    return 0
  }, [user?.tipo, adminState.aiResponses.pagination.total])
  ...
  }, [user?.tipo, pendingCount])

  return (
    <div className="border-b">
      <div className="flex h-16 items-center px-4">
        {/* Mobile menu */}
        <MobileNav navItems={navItems} pathname={pathname} pendingCount={pendingCount} />

        {/* Logo */}
        <div className="mr-6">
          <Link href="/dashboard" className="flex items-center space-x-2">
            <GraduationCap className="h-6 w-6" />
            <span className="font-bold text-xl tracking-tight hidden sm:inline-block">{appTitle}</span>
          </Link>
        </div>

        {/* Desktop navigation */}
        <div className="flex-1">
          <DesktopNav navItems={navItems} pathname={pathname} pendingCount={pendingCount} />
        </div>
        ...
        <div className="flex items-center space-x-4">
          ...
          {systemStatus && (
            <div className="hidden lg:flex items-center space-x-2">
              ...
              />
              <span className="text-xs text-muted-foreground">{systemStatus.message}</span>
            </div>
          )}
...
export function PageHeader({ title, description, action, breadcrumb }: PageHeaderProps) {
  return (
    <div className="space-y-4">
      {breadcrumb && breadcrumb.length > 0 && <BreadcrumbNav items={breadcrumb} />}
